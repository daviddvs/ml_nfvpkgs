...
#class MljujuCharm(SSHProxyCharm):
class MljujuCharm(CharmBase):
    _stored = StoredState()
    ...
    def __init__(self, *args):
        ...
        self.framework.observe(self.on.server_relation_changed, self._provide_ipaddr )
    ...
    def _provide_ipaddr(self, event):
        self.unit.status = MaintenanceStatus("Sending Server IP")
        ifname=netifaces.interfaces()[1]
        ip = netifaces.ifaddresses(ifname)[AF_INET][0]['addr']
        event.relation.data[self.unit]["ip"] = ip
        self.unit.status = ActiveStatus("Server IP sent")
